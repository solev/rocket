...
type ProgressProps = {
  ...
  trickle?: boolean;
  trickleSpeed?: number;
  customCSS?: (css: string) => React.JSX.Element;
};

const defaultConfig: Required<ProgressProps> = {
  // Use theme-aware primary color via CSS variable so it follows light/dark
  color: "#3366FF",
  startFrom: 20,
  delay: 0,
  height: 2,
...
export default function Progress(props: ProgressProps) {
  ...
  React.useEffect(() => {
    if (navigation.state !== "idle") {
      // Start immediately to avoid missing very fast navigations
      NProgress.start();
    } else {
      NProgress.done();
    }
