...
| --- | --- |
| `getPolarClient()` | throws `CapabilityUnavailableError` |
...
- **UI** — `app/routes/dashboard/dashboard.billing.tsx` renders an explicit "Billing is not configured" state. Pages are application-owned; the capability never hides or registers navigation itself.
...
`tests/unit/capability-contract.test.ts` covers the four required states against the pure `config.ts`: absent, partial (rejected by env validation — see `tests/unit/env-schema.test.ts`), unavailable invocation, and configured. No test contacts a real Polar account.

## Manual removal

1. Delete `app/capabilities/billing/`.
...
6. In `app/lib/env/schema.ts`, remove the `POLAR_*` variables and the `polar` capability group; mirror the change in `.env.example` and re-run `bun run env:check`.
7. Remove `@polar-sh/sdk` and `@polar-sh/better-auth` from `package.json`.
8. Delete the billing cases from `tests/unit/capability-contract.test.ts` and the billing assertions from `tests/e2e/smoke.spec.ts`.
